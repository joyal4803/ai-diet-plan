document.addEventListener('DOMContentLoaded', function() {
    ...
    async function downloadDietPDF() {
        ...
        try {
            ...
            const originalText = downloadDietBtn.textContent;
            ...
            if (currentDietPlan.rawResponse) {
                // For raw responses, use the markdown text directly
                dietText = currentDietPlan.rawResponse;
                console.log('Using raw response:', dietText.substring(0, 200) + '...');
            } else {
                // For structured responses, convert to readable text
                dietText = dietPlanContainer.innerText || dietPlanContainer.textContent;
                console.log('Using structured response:', dietText.substring(0, 200) + '...');
            }
            
            // If still no content, try to get it from the current diet plan object
            if (!dietText || dietText.trim() === '') {
                console.log('No text content found, trying to create from diet plan object');
                dietText = JSON.stringify(currentDietPlan, null, 2);
            }
            
            // Remove all non-ASCII (emoji/unicode) characters
            dietText = removeNonASCII(dietText);
            
            console.log('Final diet text length:', dietText.length);
            ...
            // Save the PDF
            doc.save(`diet-plan-${Date.now()}.pdf`);
